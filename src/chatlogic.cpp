...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        //auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](GraphNode* node) { return (*node).get()->GetID() == id; });
    ...
    // Instantiate CatBot on Stack
    ChatBot chatBot ("../images/chatbot.png"); 

    // Setting Chat bot Handle to ChatLogic
    SetChatbotHandle(&chatBot);
    // Set Logic Handle for chatBot
    chatBot.SetChatLogicHandle(this);
    // Set the root node for chatBot
